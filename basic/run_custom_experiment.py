...
def run_custom_experiment(
    ...
    with catchtime() as loading_time:
        ...
        # )
        for train_dataset in config_to_execute.train_dataset:
            datasets[train_dataset] = load_datasets(
                dataset_locations=dataset_locations,
                datasets_to_load=[train_dataset],
                features=config_to_execute.extra.in_use_features,
            )
        ...
        for test_dataset in config_to_execute.test_dataset:
            datasets[test_dataset] = load_datasets(
                dataset_locations=dataset_locations,
                datasets_to_load=[test_dataset],
                features=config_to_execute.extra.in_use_features,
            )
    ...
    for train_dataset, test_dataset in zip(config_to_execute.train_dataset, config_to_execute.test_dataset):
        all_results = [
            do_classification(
                datasets=datasets,
        ...
        results_per_dataset[train_dataset.split('.')[0]] = all_results
